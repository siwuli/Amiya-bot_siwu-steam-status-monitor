# -*- coding: utf-8 -*-
"""成就监控模块（移植自 astrbot_plugin_steam_status_monitor，httpx 已替换为 aiohttp）"""
import json
import os
import io
...
class AchievementMonitor:
    ...
    async def get_player_achievements(self, api_key: str, group_id: str, steamid: str, appid: int) -> Optional[Set[str]]:
        ...
        for lang in lang_list:
            ...
            for attempt in range(3):
                try:
                    resp = await http_get(url, params=params, proxy=self.proxy, timeout=15)
                    if resp is None:
                        print(f"请求失败（无响应） (第{attempt+1}次, lang={lang})")
                        continue
                    if resp.status == 200:
                        ...
                        print(f"获取成就数据失败: HTTP {resp.status} (第{attempt+1}次, lang={lang})")
                except Exception as e:
    ...
    async def get_achievement_details(self, group_id: str, appid: int, lang: str = "schinese", api_key: str = "", steamid: str = "") -> Dict[str, Any]:
        ...
        for try_lang in lang_list:
            ...
            try:
                ...
                if resp is None:
                    print(f"获取成就schema失败: 无响应 url={url}")
                    continue
                if resp.status == 400:
                    ...
                    for lang2 in lang_list:
                        ...
                        resp2 = await http_get(
                            ...
                            params=params, proxy=self.proxy, timeout=15
                        )
                        if resp2 and resp2.status == 200:
                            try:
                                ...
                                print(f"降级解析GetPlayerAchievements json失败: {e}")
                        else:
                            status = resp2.status if resp2 else "无响应"
                            print(f"降级GetPlayerAchievements失败: HTTP {status}")
                    return details
                if resp.status != 200:
                    print(f"获取成就schema失败: HTTP {resp.status} url={url}")
                    continue
                try:
                    schema = await resp.json()
                except Exception as e:
                    print(f"解析成就schema json失败: {e}")
                    continue
                achievements = {}
                ...
                resp2 = await http_get(url_stats, proxy=self.proxy, timeout=15)
                if resp2 is None or resp2.status != 200:
                    status = resp2.status if resp2 else "无响应"
                    print(f"获取成就解锁率失败: HTTP {status} url={url_stats}")
                    percents = {}
                else:
                    try:
                        stats = await resp2.json()
                    except Exception as e:
                        print(f"解析成就解锁率json失败: {e}")
                        stats = {}
                    percents = {}
    ...
    async def render_achievement_image(self, achievement_details: dict, new_achievements: set, player_name: str = "", steamid: str = None, appid: int = None, unlocked_set: set = None, font_path=None) -> bytes:
        ...
        if unlocked_set is None:
            # 无入参时无法获取玩家成就，进度归零（由调用方传入 unlocked_set）
            unlocked_set = set()
        ...
        for apiname in new_achievements:
            ...
            if icon_url:
                try:
                    icon_resp = await http_get(icon_url, proxy=self.proxy, timeout=15)
                    if icon_resp and icon_resp.status == 200:
